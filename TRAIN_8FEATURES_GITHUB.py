...
#epochs = 1000
epochs = 50
batch_size = 32 # larger than 12 or so seems to give out of memory error?
################################################################################
...
x = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)
x = Conv1D(8, kernel_size=(3), strides=(1), padding="same", activation="relu", data_format="channels_last")(x)
encoded = MaxPooling1D(pool_size=(2), strides=(4), padding='same', data_format="channels_last")(x)


x = Conv1D(8, kernel_size=(3), strides=( 1), padding='same', activation='relu', data_format="channels_last")(encoded)
...
X = np.zeros((len(all_frames),) + desired_im_sz);
...
weights_file = os.path.join(weights_dir, 'autoencoder_texture_weights_LMT_8FEATURES.hdf5')  # where weights will be saved
json_file = os.path.join(weights_dir, 'autoencoder_texture_model_LMT_8FEATURES.json')

csv_logger = CSVLogger(os.path.join(weights_dir, 'training_log_LMT_8FEATURES.csv'), append=True, separator=';') # added by KS
...
#### save model keras
autoencoder.save('autoencoder_model_LMT_8FEATURES.h5')
encoder.save('encoder_model_LMT_8FEATURES.h5')
...
if not os.path.exists('./recontructed_LMT_8FEATURES/'): os.mkdir('./recontructed_LMT_8FEATURES/')
if not os.path.exists('./Code_LMT_8FEATURES/'): os.mkdir('./Code_LMT_8FEATURES/')
# Save text file with ordered file names, as they are ordered when reconstructed - for later analyses
with open('imageListLMT_8FEATURES.txt', 'w') as f:
...
for i in range(X.shape[0]):
    scipy.io.savemat("{}{}{}".format('./recontructed_LMT_8FEATURES/', i, '.mat'), mdict={'arr': decoded_imgs[i]})
...
for i in range(2160):
    scipy.io.savemat("{}{}{}".format('./Code_8FEATURES/', i, '.mat'), mdict={'arr': code[i]})
