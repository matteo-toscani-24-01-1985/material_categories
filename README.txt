...
The reconstructed signals are saved in the "./recontructed_LMT_XXFEATURES/" folders (XX could be 8, 16 or 32 features); in the "./ReconstructedVAE16D" for the VAE network. Their size is the one of the original signals
...
THe matlab file "FORTMAT_LTM_DATABASE_GITHUB.m" takes as input the LTM databased unzipped folder from https://zeus.lmt.ei.tum.de/downloads/texture/download/LMT_108_SurfaceMaterials_Database_V1.1.zip and renders the processed files in the "./LMT_FORMATTED/" folder
the formatted data is high passed to removed frequencies below 10Hz, and normalized to forced the full range between 0 and 1. This is done with the same linear transfomration applied to all samples, which is reversed in Figure 1B
