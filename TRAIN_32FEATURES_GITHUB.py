...
x = Conv1D(32, kernel_size=(3), strides=( 1), padding='same', activation='relu', data_format="channels_last")(encoded)
...
weights_file = os.path.join(weights_dir, 'autoencoder_texture_weights_LMT_32FEATURES.hdf5')  # where weights will be saved
json_file = os.path.join(weights_dir, 'autoencoder_texture_model_LMT_32FEATURES.json')

csv_logger = CSVLogger(os.path.join(weights_dir, 'training_log_LMT_32FEATURES.csv'), append=True, separator=';') # added by KS
...
#### save model keras
autoencoder.save('autoencoder_model_LMT_32FEATURES.h5')
encoder.save('encoder_model_LMT_32FEATURES.h5')
...
if not os.path.exists('./recontructed_LMT_32FEATURES/'): os.mkdir('./recontructed_LMT_32FEATURES/')
if not os.path.exists('./Code_LMT_32FEATURES/'): os.mkdir('./Code_LMT_32FEATURES/')
...
for i in range(X.shape[0]):
    scipy.io.savemat("{}{}{}".format('./recontructed_LMT_32FEATURES/', i, '.mat'), mdict={'arr': decoded_imgs[i]})
...
for i in range(2160):
    scipy.io.savemat("{}{}{}".format('./Code_LMT_32FEATURES/', i, '.mat'), mdict={'arr': code[i]})
